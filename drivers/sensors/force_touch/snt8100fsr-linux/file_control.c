...
static const uint16_t Grip_Calibration_addr = 0x34;
...
static void Grip_K_data_recovery(void){
	...
	if(Grip_B1_F_value == 0 || Grip_B2_F_value == 0){
		...
		write_register(snt8100fsr_g, Grip_Calibration_addr, &K_data);
	}
}
...
void grip_dump_status_func(struct work_struct *work){
	...
	if(grip_status_g->G_SQUEEZE1_EN == 1)
	  grip_squeeze1_enable_func(grip_status_g->G_SQUEEZE1_EN);
...
static void Grip_Apply_Golden_K(void){
	...
	ret = write_register(snt8100fsr_g, Grip_Calibration_addr, &Golden_K);
	if (ret) {
		PRINT_CRIT("set 0x%x init failed", Grip_Calibration_addr);
	}
}
...
static void GripK_magnification(){
	...
	write_register(snt8100fsr_g, Grip_Calibration_addr, &Real_K);
