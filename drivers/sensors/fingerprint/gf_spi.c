...
static void FOD_AUTH(struct work_struct *work) {
    ...
    if (gf_dev->timer_init | g_notify_fod) {
...
void TP_call_FOD(int id, bool down){
    ...
    if (!gf_dev->lhbm_exi2_gpio | gf_dev->fb_black | panelOff | !g_notify_fod | !g_bl_lvl ) {
        if (id == TP_gesture) {
            if (down)
                g_id_fod = TP_gesture;
            else 
